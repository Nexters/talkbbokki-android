...
@Inject constructor(
    ...
    override fun getCommentList(topicId: Int): Flow<List<com.hammer.talkbbokki.presentation.comment.Comment>> =
        flow {
            emit(
                service.getComments(topicId).result.contents.map { it.toModel() }
            )
        }
